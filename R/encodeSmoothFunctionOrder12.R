...
#' 
#' @description Encodes a smooth function by its first and second derivatives. At each point on
...
encodeSmoothFunctionOrder12 <- function( smoothFunc, abscissa ){
  V1 <- V2 <- NULL
  ...
  AllStates <- getEncodingStatesOrder12()
  encodedFunc$State <- apply( X = encodedFunc[, c( 'D1', 'D2' ) ], MARGIN = 1, 
...
getEncodingStatesOrder12 <- function(){
